...
###' 
###' Task: Install NIMBLE and run the basic examples
###'       
###' Data: Simulated data
###' 
###' Date: 
...
###' Define the model code for MCMC
###' x[i] is the number of failures recorded 
###' during a time duration of length t[i] for the ith pump
pumpCode <- nimbleCode({
...
### Define contants, data, and initial values
...
###'#######################################################################
###'
###' (3) One-line invocation of MCMC
###'
###'

###' Excute two MCMC chains of 10,000 samples each
###' and return samples, summary statistics, and WAIC values
mcmc.out <- nimbleMCMC(code = pumpCode, 
                       constants = pumpConsts,
                       data = pumpData, 
                       inits = pumpInits,
                       nchains = 2, 
...
###'#######################################################################
###'
###' (4) Creating, compiling, and running a basic MCMC configuration
###'
###'

### The default sampler is a random walk Metropolis sampler.
...
###'#######################################################################
###'
###' (6) Customizing the MCMC
###' 
###'

###' Add an adaptive block sampler on alpha and beta jointly 
###' and see if that improves the mixing.
